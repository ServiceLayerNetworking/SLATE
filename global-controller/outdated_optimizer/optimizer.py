...
def run_optimizer(traces, NUM_REQUESTS):
            ...
            for svc_name in unique_services:
                ...
                for j in range(num_data_point):
                    if svc_name == INGRESS_GW_NAME:
                        ...
                        # app.logger.info(f"{gc.log_prefix} Non-Ingress gateway Service,{svc_name}, ct: {ct}, load: {load[j]}")
                    comptime.append(ct)
                    ld.append(load[j])
                ...
                app.logger.info(f"{gc.log_prefix} Service,{svc_name}, Load: {ld}")
                app.logger.info(f"{gc.log_prefix} Service,{svc_name}, Compute time: {comptime}\n")
    ...
    for cid in range(NUM_CLUSTER):
        ...
        for svc_name in unique_services:
            ...
            coef = regressor_dict[svc_name]["linearregression"].coef_
            intercept = regressor_dict[svc_name]["linearregression"].intercept_
            r_2 = np.round(r2_score(y_test, y_pred),5)
            if regressor_dict[svc_name]["linearregression"].coef_ < 0:
                app.logger.info(f"{gc.log_prefix} Service,{svc_name}, Negative Cofficient,{coef}")
                assert regressor_dict[svc_name]["linearregression"].coef_ >= 0
            if regressor_dict[svc_name]["linearregression"].intercept_ < 0:
                app.logger.info(f"{gc.log_prefix} Service,{svc_name}, Negative Intercept,{intercept}")
                assert regressor_dict[svc_name]["linearregression"].coef_ >= 0
            app.logger.info(f"{gc.log_prefix} Service {svc_name}, model slope: {coef}, intercept: {intercept}, R^2: {r_2}")
            ## Plot
