...
''' capacity in waterfall and slate 
read_config_file() sets CAPACITY
If it is runtime -> training_phase() -> calc_max_load_per_service() -> set max_load_per_service[svc] = CAPACITY
'''
CAPACITY = 0
            ...
            with stats_mutex:
                for span in spans:
                    if is_span_existed_in_trace(all_traces, span):
                        logger.info(f"{cfg.log_prefix} span already exists in all_trace {span.trace_id}, {span.span_id}, {span.svc_name}")
                    else:
