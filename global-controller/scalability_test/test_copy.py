...
def run_optimizer(coef_dict, \
    ...
    # try:
    compute_df = opt_func.create_compute_df(compute_arc_var_name, ep_str_callgraph_table, coef_dict, max_capacity_per_service, degree, endpoint_level_rps, agg_total_endpoint_rps)
    # except Exception as e:
    ...
    #     incoming_sum = 0
    #     for cid in endpoint_to_placement[endpoint]:
    #         start_node = opt_func.get_start_node_name(endpoint, cid)
    #         incoming_sum += aggregated_load.sum('*', start_node)
    #     gurobi_model.addConstr(incoming_sum <= total_root_node_rps, name="tree_topo_conservation_"+start_node)
    # gurobi_model.update()
